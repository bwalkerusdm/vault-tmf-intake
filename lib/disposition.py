...
def hold(item, eval_item, record, token, *, folders) -> dict:
    ...
    # not_tmf = wrong door (remove from intake); flagged = fixable (fix + re-file).
    # Route not_tmf to a dedicated rejected folder IF one is configured; otherwise
    # everything non-clean lands in exceptions (backwards-compatible default).
    routed_to_rejected = record.get("status") == "not_tmf" and bool(folders.get("rejected"))
    dest = folders["rejected"] if routed_to_rejected else folders["exceptions"]
    dest_label = "box:0-Rejected" if routed_to_rejected else "box:3-Exceptions"
    kind = "rejected_not_tmf" if record.get("status") == "not_tmf" else "exception"
