...
OUTOFSCOPE = os.environ.get("BOX_OUTOFSCOPE_FOLDER_ID")  # optional: out-of-scope/misrouted docs route here if set
_FOLDERS = {"inbox": INBOX, "filed": FILED, "exceptions": EXCEPTIONS}
if REJECTED:
    _FOLDERS["rejected"] = REJECTED
if OUTOFSCOPE:
    _FOLDERS["out_of_scope"] = OUTOFSCOPE
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if outcome["action"] == "filed":
            ...
            rec = record.get("checks", {}).get("record_resolution", {})
            resp.update({"disposition_kind": outcome.get("disposition_kind"),
                         "reasons": outcome["reasons"], "flag_rules": outcome["flag_rules"]})
            if rec.get("scope") and rec.get("scope") != "in_scope":
                resp["scope"] = rec.get("scope")
                resp["scope_reason"] = rec.get("scope_reason")
