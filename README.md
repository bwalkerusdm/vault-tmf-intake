...
- `/api/etmf/actions/reset_demo`  `{confirm:true, vault:true|false}` → returns the
  demo to its starting state so the SAME docs re-file fresh: moves filed/held files
  back to 1-Inbox (un-renamed, companions cleared) and, only when gated, deletes the
  agent-created Vault docs so re-filing doesn't duplicate.
- `/api/etmf/actions/report_unprocessable`  `{box_file_id, filename, reason}` → the
  agent calls this when it CANNOT process a document (unreadable, timed out,
  unsupported); holds it in 3-Exceptions with a reason + audit event, so a failure is
  a documented exception rather than a silent skip.
...
lib/   run_all_checks.py (gate_from_findings + the deterministic gate)
       ...
       reset.py (demo reset: Box folders back to start)
       vault_filing.py, box_client.py, disposition.py, audit.py (stdout), archive.py
api/etmf/actions/  list_inbox.py, file_or_flag.py, expand_archive.py, reset_demo.py
```
